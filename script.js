drawThing = function(client, context, thing, vertexes, scale, contextFn) {
  ...
  context.beginPath();
  context.moveTo( (thing.x + (start[0] * scale)) * Thing.ratio , (thing.y + (start[1] * scale)) * Thing.ratio );
  for (var i = 0; i < vertexes.length; i++) {
    var vertex = vertexes[i];
    context.lineTo((thing.x + (vertex[0] * scale)) * Thing.ratio , (thing.y + (vertex[1] * scale)) * Thing.ratio );
  }
  context.lineTo(  (thing.x + (start[0] * scale)) * Thing.ratio, (thing.y + (start[1] * scale)) * Thing.ratio );
  context[contextFn]();
  context.closePath();
}
...
drawThing2 = function(client, context, thing, vertexes, scale, contextFn) {
  var start = vertexes[0];
  context.beginPath();
  context.moveTo( (start[0] * scale) * Thing.ratio, (start[1] * scale) * Thing.ratio );
  for (var i = 0; i < vertexes.length; i++) {
    var vertex = vertexes[i];
    context.lineTo( ((vertex[0] * scale)) * Thing.ratio, ((vertex[1] * scale)) * Thing.ratio);
  }
  context.lineTo((start[0] * scale) * Thing.ratio,(start[1] * scale) * Thing.ratio);
  context[contextFn]();
  context.closePath();
}
